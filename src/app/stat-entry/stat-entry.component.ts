...
export class StatEntryComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    this.scoring = await this.firestore
      ...
      .pipe(take(1))
      .toPromise();
    this.scoringKeys = _.orderBy(Object.keys(this.scoring), (key) =>
      this.statsSorter.indexOf(key)
    );
    ...
    this.aliveTeams$ = this.teams$.pipe(
      map((teams) => teams.filter((team) => !team.eliminated))
    );
    ...
    this.selectedPlayers$ = combineLatest([
      ...
      map(([playerTeam, entries, teams, players]) => {
        const entryPlayers = _.uniq(
          _.flatten(entries.map((entry) => entry.players))
        );
        ...
        return _.orderBy(
          entryPlayers
            ...
            .filter((player) => {
              const playerTeamObj = teams.find(
                (team) => team.abbr == player.team
              );
              return playerTeamObj
                ? !playerTeamObj.eliminated && playerTeam === playerTeamObj.abbr
                : false;
            }),
          'name',
          'asc'
    ...
    this.existingStats$ = combineLatest([
      this.stats$,
      this.players$,
      this.playerTeam$,
    ]).pipe(
  ...
  async editExistingStat(stat) {
    await this.firestore
      .doc(`stats/${stat.id}`)
      .update({ statLine: stat.statLine, timestamp: Date.now() });
  }
}
